...
public class MaSangSangMaStrategy extends FormationStrategy {
    @Override
    protected Map<Position, Piece> generateHanFormation() {
        return Map.of(
                HAN_LEFT_OUTER, new Horse(Team.HAN),
                HAN_LEFT_INNER, new Elephant(Team.HAN),
                HAN_RIGHT_INNER, new Elephant(Team.HAN),
                HAN_RIGHT_OUTER, new Horse(Team.HAN)
        );
    }
    ...
    protected Map<Position, Piece> generateChoFormation() {
        return Map.of(
                ...
                CHO_RIGHT_INNER, new Elephant(Team.CHO),
                CHO_RIGHT_OUTER, new Horse(Team.CHO)
        );
    }
}
